...
void Renderer::Copy(const Texture &texture, const std::optional<Rectangle> &src,
                    const Rectangle &dst) {
  src.has_value() ? SDL_RenderCopy(renderer_.get(), texture.texture_.get(),
                                   &src->rect_, &dst.rect_)
                  : SDL_RenderCopy(renderer_.get(), texture.texture_.get(),
                                   nullptr, &dst.rect_);
}
} // namespace nge::sdl